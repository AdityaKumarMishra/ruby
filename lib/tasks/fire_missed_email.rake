namespace :fire_missed_email do
  task fetch_details: :environment do
    puts "=============== Task Start ==============="

    users = User.where('created_at BETWEEN ? AND ?', ("10/09/2021").to_date.beginning_of_day, ("16/09/2021").to_date.end_of_day)
    users.each do |user|
    ...
    # essay_responses = EssayResponse.where('time_submited BETWEEN ? AND ?', ("10/09/2021").to_date.beginning_of_day, ("16/09/2021").to_date.end_of_day)
    ...
    # essay_tutor_responses = EssayTutorResponse.where('created_at BETWEEN ? AND ?', ("10/09/2021").to_date.beginning_of_day, ("16/09/2021").to_date.end_of_day)
    ...
    # tickets = Ticket.where('created_at BETWEEN ? AND ?', ("10/09/2021").to_date.beginning_of_day, ("16/09/2021").to_date.end_of_day)
    ...
    ticket_answers = TicketAnswer.where('created_at BETWEEN ? AND ?', ("10/09/2021").to_date.beginning_of_day, ("16/09/2021").to_date.end_of_day)
