namespace :fire_08_to_09_june_failed_emails do
  task fetch_details: :environment do
    puts "=============== Task Start ==============="

    users = User.where('created_at BETWEEN ? AND ?', ("08/06/2021").to_date.beginning_of_day, ("10/06/2021").to_date.end_of_day)
    users.each do |user|
    ...
    tickets = Ticket.where('created_at BETWEEN ? AND ?', ("08/06/2021").to_date.beginning_of_day, ("10/06/2021").to_date.end_of_day)
   ...
   ticket_answers = TicketAnswer.where('created_at BETWEEN ? AND ?', ("08/06/2021").to_date.beginning_of_day, ("10/06/2021").to_date.end_of_day)
   ticket_answers.each do |ticket_answer|
    ...
    essay_responses = EssayResponse.where('created_at BETWEEN ? AND ?', ("08/06/2021").to_date.beginning_of_day, ("10/06/2021").to_date.end_of_day)
    ...
    essay_tutor_responses = EssayTutorResponse.where('created_at BETWEEN ? AND ?', ("08/06/2021").to_date.beginning_of_day, ("10/06/2021").to_date.end_of_day)
