namespace :fire_16_to_21_dec_failed_emails do
  task fetch_details: :environment do
    puts "=============== Task Start ==============="
    users = User.where('created_at BETWEEN ? AND ?', ("16/12/2020").to_date.beginning_of_day, ("21/12/2020").to_date.end_of_day)
    users.each do |user|
    ...
    tickets = Ticket.where('created_at BETWEEN ? AND ?', ("16/12/2020").to_date.beginning_of_day, ("21/12/2020").to_date.end_of_day)
    ...
    essay_responses = EssayResponse.where('created_at BETWEEN ? AND ?', ("16/12/2020").to_date.beginning_of_day, ("21/12/2020").to_date.end_of_day)
    essay_responses.each do |essay_response|
      if essay_response.status == "unmarked"
        ...
        if tutor_emails.present?
          EssayResponseMailer.unmarked_essay(essay_response, tutor_emails).deliver_later if ENV['EMAIL_CONFIRMABLE'] != "false"
        else
