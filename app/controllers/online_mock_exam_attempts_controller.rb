class OnlineMockExamAttemptsController < ApplicationController
  ...
  def index
    ...
    mock_exam = current_course.master_features.where("master_features.name LIKE?", '%OnlineMockExamFeature2%').first
    if master_feature.present? && master_feature.online_mock_exam?
      feature_acquired = current_user.accessible_features.find_by("name LIKE?", '%OnlineMockExamFeature2%')
    ...
    respond_to do |format|
      if online_mock_exam_attempt.nil? && @online_mock_exam.valid_attempt_to_create?
        ...
        if @online_mock_exam_attempt.save
          format.html { redirect_to online_mock_exam_attempts_path, notice: 'Online Mock Exam Attempt was successfully created.' }
          format.json { render :show, status: :created, location: @online_mock_exam_attempt }
        else
          format.html { redirect_to online_mock_exam_attempts_path }
        ...
        format.html { redirect_to online_mock_exam_attempts_path, notice: "Score saved successfully"}
        format.json { render json: @online_mock_exam_attempt.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to online_mock_exam_attempts_url, notice: 'Online exam attempt was successfully destroyed.' }
  ...
  def set_online_mock_exam
    # For Revert
    # @online_mock_exam = OnlineMockExam.by_product_line("Gamsat").find_by(city: OnlineMockExam.cities[current_course.city])
    @online_mock_exam = OnlineMockExam.by_product_line("Gamsat").find_by(city: OnlineMockExam.cities[current_course.city], per_city_exam_count: 2, published: true)
  end
  ...
  def set_sections
    #Added where(assessable_id: @online_mock_exam.id)) for temp
    @current_course = current_user.active_course
    if current_user.online_mock_exam_attempts.present?
