...
;(function($, window, undefined) {
 ...
 $.ZabutoCalendar.prototype = {
   ...
   _renderCalendar: function() {
     ...
     this._parseEvents(year, month, essay_status, tutor_selected);
   },
   ...
   _parseEvents: function(year, month, essay_status, tutor_selected) {
     download_sheet_data();
     this.$eventsArray = [];
     if (false !== this.$calendarElement.data('localJsonData'))
       this._parseJsonEvents();
     if (false !== this.$calendarElement.data('ajaxSettings'))
       this._parseAjaxEvents(year, month, essay_status, tutor_selected);
   },
   ...
   _parseAjaxEvents: function(year, month, essay_status, tutor_selected) {
     var ajaxSettings = this.$calendarElement.data('ajaxSettings');
   ...
   _drawCalendarEvent: function(event_object) {
     ...
     if (event_object.title == "") {
      $dayCellElement.find("i").addClass('bold_date');
     }
     else if (event_object.title >= 10) {
       $dayEventElement.addClass('badge_red_color');
     } else {
       $dayEventElement.removeClass('badge_red_color');
     }
