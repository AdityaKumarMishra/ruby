...
RSpec.describe CoursesController, type: :controller do
  ...
  describe "GET #list_course_lessons" do
    it "gets a list of course lessons without a valid course_id" do
      expect {
        get :list_course_lessons, { course_id: nil }, valid_session
      }.to raise_error(ActiveRecord::RecordNotFound)
    end

    it 'gets a list of course lessons with a valid course_id' do
      course = FactoryGirl.create(:course)
      get :list_course_lessons, course_id: course.id, format: 'json'
      expect(response.content_type).to eq 'application/json'
    end
  end

  describe "GET #list_cities" do
    it 'gets a list of courses cities in json' do
      product_version = FactoryGirl.create(:product_version)
      get :list_cities, product_id: product_version.id
      expect(response.content_type).to eq 'application/json'
    end
  end

  describe "POST #duplicate" do
    it "it will create dupliacte course" do
      course = Course.create! valid_attributes
      post :show, {:id => course.to_param}, valid_session
      expect(response).to redirect_to(courses_url)
      expect(flash[:notice]).to match(/Course is duplicated successfully/)
    end
  end
end
