...
RSpec.describe TagsController, type: :controller do
  ...
  describe "DELETE #destroy" do
    context "destroy after login admin" do
      login_admin
      it "destroys the requested tag by login admin" do
    ...
    context "destroy after login superadmin" do
      login_superadmin
      it "destroys the requested tag by login superadmin" do
        tag = FactoryGirl.create(:tag)
        expect {
          delete :destroy, {:id => tag.to_param}, valid_session
        }.to change(Tag, :count).by(-1)
  ...
  describe '#children_for_tags' do
    it 'returns public tags' do
      tag = FactoryGirl.create(:tag, is_public: true)
      public_tag = FactoryGirl.create(:tag, is_public: true, parent_id: tag.id)
      get :children_for_tags, { tag_id: tag.to_param }, format: 'json'
      expect(response.content_type).to eq 'application/json'
      expect(response.body).to eq [public_tag].to_json
    end
  end
end
